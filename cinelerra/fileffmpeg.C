...
//Video Seek //Original Monty
#define SEEK_THRESHOLD 16 //16
#define SEEK_BACK_START 24 //24
#define SEEK_BACK_LIMIT 600 //600

//Audio Seek //Original Monty
#define EPSILON .000001 // .000001
#define A_SEEK_THRESHOLD 1. //1.
#define A_SEEK_BACK_START .2 //.2
#define A_SEEK_BACK_LIMIT 5. //5.
...
int FileFFMPEG::check_sig(Asset *asset)
{
	...
	if(result >= 0)
	{
		result = av_find_stream_info(TEMPcontext);
		if(result >= 0)
		{
			av_close_input_file(TEMPcontext);
			ffmpeg_lock->unlock();
			return 1;
		}
		ffmpeg_lock->unlock();
						...
						if(video_index < 0)
						{
							...
							if(EQUIV(asset->frame_rate, 0))
							{
								...
								if(	decoder_context->coded_width == 1440 &&
									...
									decoder_context->coded_height == 1080 &&
									1. * stream->avg_frame_rate.num / 
									stream->avg_frame_rate.den <
									1. * decoder_context->time_base.den / 
							...
							asset->aspect_ratio =	(double)(decoder_context->coded_width *
...
int FileFFMPEG::read_frame(VFrame *frame)
{
	...
	int64_t stream_start = av_rescale_q(ffmpeg_file_context->start_time, 
										AV_TIME_BASE_Q,
										stream->time_base);

	int64_t target = file->current_frame / 
						asset->frame_rate *
	...
	if((file->current_frame != current_frame+1) || unsynced) 
	{
		...
		if(debug) printf("video_pts SEEK %d %d requested=%.03f (min=%0.3f) \n",
			...
			*/
			...
			if(!pre_sync)
			{
				...
				if(seekto <= stream_start+EPSILON)
				{
					...
					if(debug) fprintf(stderr,"adjusted=%.03f ",
									1.*(seekto-stream_start)*
									stream->time_base.num/
									stream->time_base.den);
				
					if(av_seek_frame(ffmpeg_file_context,video_index,seekto,AVSEEK_FLAG_BACKWARD))
					{
						error = 1;
			...
			while(!error)
			{
				...
				error = av_read_frame(ffmpeg_file_context,&vspacket);
				
				if(debug && error) fprintf(stderr,"PERROR ");
				
				if(!error && vspacket.size > 0 && vspacket.stream_index == video_index)
				{

					if(debug) fprintf(stderr,"vspacket=%.03f ", 
												1.*(vspacket.pts-stream_start)*
												stream->time_base.num/
												stream->time_base.den);
					...
					if(vspacket.flags&PKT_FLAG_KEY)
					{
						got_keyframe = 1;
						if(debug) fprintf(stderr,"KEYFRAME ");
					}
					// starting past where we want to be?
					if(!pre_sync && vspacket.pts != AV_NOPTS_VALUE && vspacket.pts > target_abort) break;
	
					if(got_keyframe)
					{
						if(!ffmpeg_frame) ffmpeg_frame = avcodec_alloc_frame();
						
						avcodec_get_frame_defaults(ffmpeg_frame);
	
						decoder_context->reordered_opaque = vspacket.pts;
						...
						if(debug) fprintf(stderr,
											">>>VID results: reordered=%.03f pts=%.03f dts=%.03f result=%d got_pic=%d %s %s %s\n",
											(ffmpeg_frame->reordered_opaque-stream_start)*
											1.*stream->time_base.num/stream->time_base.den,
											(vspacket.pts-stream_start)*
											1.*stream->time_base.num/stream->time_base.den,
											(vspacket.dts-stream_start)*
											1.*stream->time_base.num/stream->time_base.den,
											result,
											got_pic,
											((ffmpeg_frame->data[0])?"DATA":""),
											((vspacket.flags&PKT_FLAG_KEY)?"PACKETKEY":""),
											((ffmpeg_frame->key_frame)?"FRAMEKEY":""));
						...
						if(got_pic)
						{
							...
							if(debug) fprintf(stderr,"delivered=%.03f ",
							...
							{
								if (vspacket.dts != AV_NOPTS_VALUE)
								{
									returned_pts= vspacket.dts;
								}
								else
								{
									if (vspacket.dts != AV_NOPTS_VALUE)
									returned_pts= vspacket.pts;
									else
									{
							...
							{
								if(debug) fprintf(stderr,"PRESYNC \n");
								pre_sync = 1;
							}
							...
							{
								if(debug) fprintf(stderr,"POSTSYNC \n");
								got_it = 1;
								current_frame = file->current_frame;
								break;
							}
	
							if(returned_pts > target && !pre_sync) break;
						}
					}
				}
				av_free_packet(&vspacket);
			}
	...
	else
	{
		...
		while(!error)
		{
			AVPacket vnpacket;
			int got_pic = 0;
			//av_freep(ffmpeg_frame);
			error = av_read_frame(ffmpeg_file_context,&vnpacket);

			if(!error && vnpacket.size > 0)
			{
				if(vnpacket.stream_index == video_index)
				{
					if(!ffmpeg_frame) ffmpeg_frame = avcodec_alloc_frame();
					avcodec_get_frame_defaults(ffmpeg_frame);
					decoder_context->reordered_opaque = vnpacket.pts;
					int result =
#if LIBAVCODEC_VERSION_INT < ((52<<16)+(0<<8)+0)
								avcodec_decode_video(decoder_context,
														ffmpeg_frame,
					...
					if(got_pic)
					{
						...
						if(debug) fprintf(stderr,"VIDEO: nominal pts=%.03f",
											1.*returned_pts*stream->time_base.num/
											stream->time_base.den);
						current_frame++;
						got_it = 1;
						break;
	...
	{	
		int64_t returned_pts = ffmpeg_frame->reordered_opaque;
		int input_cmodel;
		//AVFrame *ffmpeg_frame = ffmpeg_frame;
		...
		switch(decoder_context->pix_fmt)
		{
			...
			default:
				if (debug) fprintf(stderr, 
									"cinelerra_ffmpeg_decode: unrecognized color model %d\n", 
		...
		for(int i = 0; i < decoder_context->height; i++)
			input_rows[i] = ffmpeg_frame->data[0] + 
				i * 
		...
		cmodel_transfer(frame->get_rows(), /* Leave NULL if non existent */
			...
			frame->get_u(),
			frame->get_v(),
			ffmpeg_frame->data[0], /* Leave NULL if non existent */
			ffmpeg_frame->data[1],
			ffmpeg_frame->data[2],
			...
			ffmpeg_frame->linesize[0],       /* For planar use the luma rowspan */
			frame->get_w());
		free(input_rows);
...
int FileFFMPEG::read_samples(double *buffer, int64_t len)
{
	...
	if(decode_start != decode_end || unsynced) 
	{
		...
		if(debug) fprintf(stderr,"audio_pts SEEK requested=%.03lf(%.03lf) ",
							1.*decode_start / asset->sample_rate,
							...
							target = 1. * decode_start * stream->time_base.den /
										stream->time_base.num /
		...
		while(!got_it && !error && seek_back <= A_SEEK_BACK_LIMIT)
		{
			...
			if(debug) fprintf(stderr,"adjusted=%.03lf ",
								1.f * (seekto-stream_start) * 
								stream->time_base.num / 
								stream->time_base.den);
			// AV_SEEK_FLAG_ANY must be set for the AVI backend to seek in audio at all
			if(av_seek_frame(ffmpeg_file_context,
							audio_index,
							seekto,
							AVSEEK_FLAG_ANY))
			{
			...
			while(!error && (!got_it || accumulation<decode_len))
			{
				...
				error = av_read_frame(ffmpeg_file_context,&apacket);
	
				if(!error && apacket.size > 0 && apacket.stream_index == audio_index)
				{
					int apacket_len = apacket.size;
					uint8_t *apacket_ptr = apacket.data;
					...
					{
						pts_sample =	1. * (apacket.pts-stream_start) *
										stream->time_base.num*asset->sample_rate/
										stream->time_base.den;
					}
	
					if(!pre_sync && apacket.pts > target)
					{
						if(seek_back*2.+EPSILON >= A_SEEK_BACK_LIMIT)
						{
							...
							int64_t padding =	(apacket.pts - stream_start) *
												asset->sample_rate * 
												stream->time_base.num / 
												stream->time_base.den -
												file->current_sample;
					...
					if(debug) fprintf(stderr,"apacket = %.03f:%d:%d:%d ",
									1.*(apacket.pts-stream_start)/
									stream->time_base.den*
									stream->time_base.num,
									error,
									apacket.size,
									apacket.stream_index);
	
					while(apacket_len > 0 && !error)
					{
						int data_size = AVCODEC_MAX_AUDIO_FRAME_SIZE;
						if(!ffmpeg_samples) ffmpeg_samples = (short*)malloc(AVCODEC_MAX_AUDIO_FRAME_SIZE);
						int bytes_decoded =
	#if LIBAVCODEC_VERSION_INT < ((52<<16)+(0<<8)+0) 
						avcodec_decode_audio2(decoder_context,
										ffmpeg_samples,
										&data_size,
										apacket_ptr,
										apacket_len);
	#else
						avcodec_decode_audio3(decoder_context,
										ffmpeg_samples,
										&data_size,
										&apacket);
	#endif
						...
						else
						{
							int samples_decoded = 	data_size /
													...
													decoder_context->sample_fmt)/8);
							
							apacket_ptr += bytes_decoded;
							apacket_len -= bytes_decoded;
							...
							if(got_it || pts_sample+samples_decoded > decode_start)
							{
								...
								if(!got_it && debug) fprintf(	stderr,
																"delivered = %.03f(%.03f) ",
																1.*(apacket.pts-stream_start)/
																	stream->time_base.den*
																	stream->time_base.num,
		...
		while(!error && accumulation<decode_len)
		{
			...
			if(!error && apacket.size > 0 && apacket.stream_index == audio_index)
			{
				...
				else
				{
					...
					if(debug && accumulation==0) fprintf(stderr,
												"nominal = %.03f(%.03f) ",
												1.*(apacket.pts-stream_start)/
													stream->time_base.den*
													stream->time_base.num,
